...
public class PlaceRectSinglePlayer : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        soundPlaceRectObj = GameObject.Find("SoundPlaceRect");
        soundPlaceRect = soundPlaceRectObj.GetComponent<AudioSource>();
        gameManagerScript = gameManager.GetComponent<GameManagerSinglePlayer>();
        passMoveScript = passMove.GetComponent<PassMoveSinglePlayer>();
        ...
        if (!(myself.name.Contains("SinglePixel"))){
            DrawLinesSinglePlayer.toDrawOutlineForRect = true;
        }
        else
        {
            placed = true;
